...
define(["avalon"], function() {
    ...
    var lazyload = avalon.bindingHandlers.lazyload = function(data, vmodels) {
        ...
        // 设置placeholderImg
        options.placeholderImg = element.getAttribute("data-lazyload-placeholder-img") === null ? options.placeholderImg : element.getAttribute("data-lazyload-placeholder-img")
        ...
        if (element.tagName === "IMG") {
            element.src = path ? path + options.placeholderImg : options.placeholderImg
        }
    ...
    var _renderImg = function(ele, src, isloadingOriginal, needResize, tempImgItem) {
        ...
        if (ele.tagName !== "IMG" && isloadingOriginal) {
            var domContent = src
            src = path ? path + options.placeholderImg : options.placeholderImg
        }
    ...
    lazyload.defaults = {
        ...
        preLoadSrc: path ? path + "/images/loading1.gif" : "/images/loading1.gif", //@config  预加载图片路径（文字内容）：preLoadType为"image"时为图片路径；preLoadType为"text"时为文字内容。也可以设置元素的data-lazyload-preloadsrc，替代默认值
