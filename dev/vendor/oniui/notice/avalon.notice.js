...
define(["../avalon.getModel", "text!./avalon.notice.html", "css!../chameleon/oniui-common.css", "css!./avalon.notice.css"], function(avalon, sourceHTML) {
    ...
    var widget = avalon.ui.notice = function(element, data, vmodels) {
        ...
        var vmodel = avalon.define(data.noticeId, function(vm) {
            ...
            vm._close = function() { //close按钮click时的监听处理函数
                var $rootElement = avalon(vm.rootElement)
                vm.toggle && $rootElement.addClass('fadeoutupbig')
            }
            vm.animationend = function(e) {
                avalon(vm.rootElement).removeClass(e.animationName)
                if (e.animationName === 'fadeoutupbig') {
                    vmodel.toggle = false
                }
            }
        ...
        vmodel.$watch("warningClass", function(v) {
            vmodel.typeClass = v
        })
    ...
    widget.defaults = {
         ...
         */
        onHide: avalon.noop,
        warningClass: "", //@config type为warning时的提示类名
